// Every hardcoded value in the prototype lives here, typed against src/types.js.
// PHASE 2: replace this folder with the real data layer; nothing else changes.
...
export const ROLES = [
  { id: 'repair_manager', label: 'Tyre repair manager', world: 'field', does: 'Triage tyres at the mine, start the jobs', entry: 'field' },
  { id: 'repairer', label: 'Workshop repairer', world: 'workshop', does: 'Advance stages, stage photos, cook time', entry: 'console' },
  { id: 'supervisor', label: 'Workshop supervisor', world: 'workshop', does: 'Receive goods, run the branch queue, override a blocked stage', entry: 'console' },
  { id: 'ndt_tech', label: 'NDT technician', world: 'workshop', does: 'Blast, test, record cracks, issue certificates', entry: 'ndt' },
  { id: 'sales', label: 'Sales', world: 'office', does: 'Read the job, raise the quote in NetSuite, answer the mine', entry: 'console' },
  { id: 'super', label: 'Superintendent', world: 'office', does: 'Cross-branch performance', entry: 'performance' },
];

const p = (id, kind, stage, capturedBy, capturedAt, where, note, queued) =>
  ({ id, kind, stage, capturedBy, capturedAt, where, note, queued: !!queued });

// ── Mackay tyre jobs ─────────────────────────────────────────────────────────
export const TYRE_JOBS = [
  {
    jobNo: 'MKY-TR-26-0417', rail: 'tyre', branch: 'MKY', customer: 'Kurrajong Coal',
    site: 'Kurrajong Coal — Ridgeview Pit', visitNo: 'SV-26-0088',
    asset: { id: 'a1', kind: 'tyre', serial: 'BR7K48219', make: 'Bridgestone', size: '40.00R57', fleetNo: 'HT-412',
      history: [
    ...
    stage: 'cooked', category: 'Major', damagePosition: 'sidewall', damageSizeMm: 168, beltPlyDamage: true,
    repairUnit: 'Section repair — 220 × 160 mm', cookSeconds: 5400, cookRemaining: 4360,
    quote: { number: 'EST-10482', status: 'Revised', history: [
  ...
  },
  {
    ...
    stage: 'inspected', category: 'Major', damagePosition: 'shoulder', damageSizeMm: 205, beltPlyDamage: true,
...
// ── Mackay rim jobs ──────────────────────────────────────────────────────────
export const RIM_JOBS = [
  {
    jobNo: 'MKY-RJ-26-0132', rail: 'rim', branch: 'MKY', customer: 'Kurrajong Coal',
    site: 'Kurrajong Coal — Ridgeview Pit',
    asset: { id: 'r1', kind: 'rim', serial: 'RM-88-2214', customerAssetNo: 'R-4471', size: '63 in — 5-piece',
    ...
    events: [], photos: [p('ph-0132-c1', 'crack', 'ndt_tested', 'A. Petrov', '08 Aug 26 · 09:31', 'Mackay', 'Gutter, 62 mm'),
...
export const ALL_JOBS = [...TYRE_JOBS, ...RIM_JOBS, ...CLOSED_JOBS];

export const jobsForBranch = (code) => (code === 'MKY' ? [...TYRE_JOBS, ...RIM_JOBS] : []);
...
const clean = (row, asset, serial, size, fleet, hours, date) =>
...
// ── Branch performance ───────────────────────────────────────────────────────
export const BRANCH_REPORTS = [
  { branch: 'MKY', minors: 14, intermediates: 31, majors: 18, jobsClosed: 63, manHours: 1200, repairHours: 600, rejectionRate: 0.07,
    avgStageHours: { received: 6, washed: 9, inspected: 14, cut_out: 11, repaired: 19, cooked: 2 } },
  { branch: 'BLW', minors: 9, intermediates: 18, majors: 7, jobsClosed: 34, manHours: 640, repairHours: 402, rejectionRate: 0.05,
    avgStageHours: { received: 5, washed: 7, inspected: 12, cut_out: 10, repaired: 16, cooked: 2 } },
  { branch: 'KAL', minors: 12, intermediates: 24, majors: 12, jobsClosed: 48, manHours: 880, repairHours: 511, rejectionRate: 0.09,
    avgStageHours: { received: 8, washed: 11, inspected: 15, cut_out: 13, repaired: 21, cooked: 2 } },
  { branch: 'BNE', minors: 7, intermediates: 11, majors: 4, jobsClosed: 22, manHours: 410, repairHours: 268, rejectionRate: 0.04,
    avgStageHours: { received: 4, washed: 6, inspected: 10, cut_out: 9, repaired: 14, cooked: 2 } },
  { branch: 'MUS', minors: 6, intermediates: 13, majors: 6, jobsClosed: 25, manHours: 470, repairHours: 300, rejectionRate: 0.06,
    avgStageHours: { received: 6, washed: 8, inspected: 13, cut_out: 10, repaired: 17, cooked: 2 } },
  { branch: 'COB', minors: 4, intermediates: 8, majors: 3, jobsClosed: 15, manHours: 300, repairHours: 181, rejectionRate: 0.08,
    avgStageHours: { received: 7, washed: 9, inspected: 14, cut_out: 12, repaired: 18, cooked: 2 } },
  { branch: 'WWY', minors: 3, intermediates: 6, majors: 2, jobsClosed: 11, manHours: 220, repairHours: 140, rejectionRate: 0.05,
    avgStageHours: { received: 5, washed: 7, inspected: 11, cut_out: 9, repaired: 15, cooked: 2 } },
  { branch: 'LTN', minors: 0, intermediates: 0, majors: 0, jobsClosed: 0, manHours: 0, repairHours: 0, rejectionRate: 0,
    avgStageHours: {} },
];
