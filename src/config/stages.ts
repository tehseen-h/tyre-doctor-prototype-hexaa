...
// captures and notifications. Ported from project/src/config/stages.js.
// Components read stage order from here; this table is also the state machine.
...
export const TYRE_STAGES: StageDef[] = [
  {
...
export const TYRE_TERMINAL: StageDef = {
...
export const RIM_STAGES: StageDef[] = [
  {
...
export const stageById = (id: string): StageDef | null =>
  TYRE_STAGES.find((s) => s.id === id) ||
  RIM_STAGES.find((s) => s.id === id) ||
  (TYRE_TERMINAL.id === id ? TYRE_TERMINAL : null);

export const railStages = (rail: 'tyre' | 'rim'): StageDef[] => (rail === 'rim' ? RIM_STAGES : TYRE_STAGES);

export const stageIndex = (rail: 'tyre' | 'rim', id: string): number => railStages(rail).findIndex((s) => s.id === id);

export const nextStageId = (rail: 'tyre' | 'rim', id: string): string | null => {
  const stages = railStages(rail);
  const i = stageIndex(rail, id);
  if (i === -1 || i >= stages.length - 1) return null;
  return stages[i + 1].id;
};

export const prevStageId = (rail: 'tyre' | 'rim', id: string): string | null => {
  const stages = railStages(rail);
  const i = stageIndex(rail, id);
  if (i <= 0) return null;
  return stages[i - 1].id;
